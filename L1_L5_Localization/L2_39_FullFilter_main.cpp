...
vector<float> initialize_priors(int map_size, 
                                vector<float> landmark_positions,
                                float position_stdev);

float motion_model(float pseudo_position, 
                   float movement, 
                   vector<float> priors,
                   int map_size, 
                   int control_stdev);
...
float observation_model(vector<float> landmark_positions, 
  ...
  for (int z=0; z< observations.size(); ++z) {
    ...
    distance_prob *= normpdf(observations[z], pseudo_range_min,
                                      observation_stdev);
  }
...
float motion_model(float pseudo_position, float movement, vector<float> priors,
  ...
  for (float j=0; j< map_size; ++j) {
    ...
    float transition_prob = normpdf(distance_ij, movement, 
                                             control_stdev);
