...
WEIGHTED_COST_FUNCTIONS = [
    ...
    (efficiency_cost,   1),
    (max_jerk_cost,     10000),
    (total_jerk_cost,   1000),
    (collision_cost,    10000),
    (buffer_cost,       10),
    (max_accel_cost,    10000),
    (max_accel_cost_d,  10000),
    (total_accel_cost,  50),
    (total_accel_cost_d,1000),
]
...
def PTG(start_s, start_d, target_vehicle, delta, T, predictions):
    ...
    arguments:
     ...
     delta - [s, s_dot, s_double_dot, d, d_dot, d_double_dot]
     ...
     predictions - dictionary of {v_id : vehicle }. 
       Each vehicle has a method vehicle.state_in(time) which returns a length 
       6 array giving that vehicle's expected [s, s_dot, s_ddot, d, d_dot, d_ddot] 
       state at that time.

    Return:
     (best_s, best_d, best_t) where 
        best_s are the 6 coefficients representing s(t)
        best_d are the 6 coefficients representing d(t)
    ...
    while t <= T + 4 * timestep:
        ...
        goals = [(goal_s, goal_d, t)]
            #goals stores all perturbed goal_s, goal_d, and its corresponding time, t, to reach the goal
        for _ in range(N_SAMPLES):
            perturbed = perturb_goal(goal_s, goal_d) # add sigma to the goal
...
def calculate_cost(trajectory, target_vehicle, delta, goal_t, predictions, cost_functions_with_weights, verbose=False):
    """Calcualate the cost for each input trajectories
    ...
    Args:
        ...
        cost_functions_with_weights (list): the weights to each cost function
        verbose (bool, optional): whether or not would like to print the cost for the input trajectories. Defaults to False.

    Returns:
        cost(float): the the cost for the input trajectories.
    """
...
def perturb_goal(goal_s, goal_d):
    """
    arguments:
     goal_s - [s, s_dot, s_double_dot], no sigma

     goal_d - [d, d_dot, d_double_dot], no sigma

    Returns:
      "perturbed" version of the goal. Add sigma to goal_s & goal_d
    """
    new_s_goal = []
...
def JMT(start, end, T):
    ...
    return:
        alpha(nparray): [alpha_0,alpha_1,alpha_2,alpha_3,alpha_4,alpha_5]
    """
