...
class Vehicle(object):
    ...
    def state_in(self, t):
        ...
        state = [
            s[0] + (s[1] * t) + s[2] * t**2 / 2.0, #s
            s[1] + s[2] * t, #s_dot
            s[2], #s_double_dot
            d[0] + (d[1] * t) + d[2] * t**2 / 2.0, #d
            d[1] + d[2] * t, #d_dot
            d[2], #d_double_dot
        ]
        return state
...
def to_equation(coefficients):
    ...
    Args:
        coefficients(tuple): coeff for s or d in format as following:
...
def nearest_approach(traj, vehicle):
    """
    Calculates the closest distance to a specific vehicle(from prediction) 
    during a trajectory.
    """
    closest = 999999
    s_,d_,T = traj
...
def show_trajectory(s_coeffs, d_coeffs, T, vehicle=None):
    ...
    plt.xlabel("s")
    plt.ylabel("d")
    plt.legend(["Self-Driving Car","Traffic"])
    plt.show()
